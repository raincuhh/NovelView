...
export class AvatarsStorageAdapter implements StorageAdapter {
	...
	async uploadFile(
		...
		const res = await this.options.client.storage.from(AppConfig.buckets.avatars).upload(filename, blob, {
	...
	async downloadFile(filePath: string): Promise<Blob> {
		...
		const { data, error } = await this.options.client.storage
			.from(AppConfig.buckets.avatars)
			.download(filePath);
